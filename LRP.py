...
class LRP():
    ...
    def generate(self, input_image, target_class):
        layers_in_model = list(self.model._modules['conv_layers']) + list(self.model._modules['fc_layers'])
        number_of_layers = len(layers_in_model)
        # Needed to know where flattening happens
        features_to_classifier_loc = len(self.model._modules['conv_layers'])

        # Forward outputs start with the input image
        forward_output = [input_image]
        # Then we do forward pass with each layer
        for conv_layer in list(self.model._modules['conv_layers']):
...
if __name__ == '__main__':
    ...
    lrp_to_vis = np.array(LRP_per_layer[10][0]).sum(axis=0)
    ...
    heatmap.figure.savefig('/Users/adamwalker/Github_Desktop/NYU_Deep_Learning/pytorch-cnn-visualizations/results/LRP_out.png')
